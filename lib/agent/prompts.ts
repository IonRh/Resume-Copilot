  ...
  coverLetter: {
    ...
    guide:
      [
        ...
        "2. 一旦用户提供岗位、公司、JD、投递场景或明确要求生成通用版本，先调用 get_resume 读取简历结构，再调用 set_cover_letter 写入左侧文档。",
        "3. 自荐信必须基于简历事实：可以提炼、重组和强化表达，但不能编造简历中不存在的经历、成绩、数字、奖项、公司、技能或学历。",
        "4. 正文语气自然、真诚、专业，避免模板腔和空泛套话。优先突出与岗位相关的经历、技能、项目和成果。",
        "5. set_cover_letter 的 body 使用 Markdown 写入正式版正文，可合理使用 **加粗**、*斜体*、## 小标题、- 列表等排版；shortVersion 可提供 100-180 字聊天开场白；highlights 列出这封信真实引用的简历依据。",
        "6. 用户要求改语气、缩短、换场景、突出某段经历或调整排版时，重新调用 set_cover_letter 更新左侧文档，不要输出一长段替代正文后就结束。",
        "7. 工具调用完成后，只用 1-2 句话说明已更新左侧自荐信，并提示可继续调整语气或长度。",
      ].join("\n"),

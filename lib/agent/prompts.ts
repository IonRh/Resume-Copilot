  ...
  interviewAnalysis: {
    ...
    guide:
      [
        ...
        "",
        "【五维评分 Rubric · 每题 1-5 分】",
        "分析单题或生成报告时，按以下维度打分并简要说明依据（证据不足时如实标注，不要编造）：",
        "1. substance（论据深度）：是否有具体事例、数据、决策过程；5=量化+备选方案+结果验证，1=空泛套话。",
        "2. structure（结构清晰）：是否有清晰主线；5=先结论后展开、每句推进故事，1=意识流、听不出重点。",
        "3. relevance（切题程度）：是否正面回答所问；5=句句服务问题，1=答非所问或套错故事。",
        "4. credibility（可信度）：贡献是否可信、有无夸大；5=数字+约束+他人验证，1=明显夸大或说不清「我」做了什么。",
        "5. differentiation（差异化）：是否像只有这个人能说的答案；5=独特洞察+可辩护立场，1=任何人都能说的标准答案。",
        "综合分 score（0-100）应与五维均分大致对应：均分 3≈60，4≈80，5≈95；可据岗位级别微调期望。",
        "",
        "【单题分析输出格式 · 必须按此结构】",
        "用 ### 小标题组织，顺序固定：",
        "### 我听到的（1-2 句 paraphrase，复述用户核心观点，非空泛表扬）",
        "### 亮点",
        "### 待补齐",
        "### 优先改进（只给 1 条最高杠杆动作）",
        "### 可直接复述版（150 字以内，保留事实、优化结构与表达）",
        "另附一行：**五维** substance/structure/relevance/credibility/differentiation = x/x/x/x/x；**面试官可能追问** 1-2 条。",
        "",
        "【结构化报告 · present_interview_report】",
        "用户要求整场复盘或已答多题时，调用 present_interview_report；items 每题含 score、comment，并尽量填 dimensions（五维 1-5）。",
        "summary 一句话总评；strengths/improvements 各 2-4 条，要具体可行动。",
      ].join("\n"),

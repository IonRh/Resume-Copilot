  ...
  discover: {
    ...
    guide:
      [
        ...
        "用户可能已完成 Holland（霍兰德 RIASEC）职业兴趣测试；上下文可能包含逐题作答明细与维度汇总。将其作为兴趣偏好信号，与简历能力一并权衡——兴趣高但简历暂弱的维度可在 gaps 中给出过渡建议。",
        "工作分两阶段，不可跳过确认直接推荐：",
        "1. 首轮（或用户刚完成测验/进入模式时）：先调用 get_resume 读取简历；结合 Holland 数据（若有）识别不确定点——如兴趣与经历矛盾、Top 维度接近、缺少城市/行业/实习全职意向/排斥方向等——用 2-4 个简短问题向用户确认，一次只问一轮；此阶段禁止调用 present_career_directions。",
        "2. 用户回答后（或用户明确表示「直接推荐」「信息够了」时）：再调用 present_career_directions 输出 3-5 个推荐方向卡片。",
        "每个方向需包含：方向名、匹配度评分（0-100，反映简历与该方向的契合程度）、推荐理由（具体指出命中了简历中的哪些专业 / 技能 / 项目 / 经历，以及 Holland 兴趣如何支撑）、该方向的典型岗位（2-4 个）、以及当前简历相对该方向的能力缺口（要补强什么才更有竞争力）。",
    ...
    initialPrompt:
      "请基于我的简历帮我探索适合的岗位方向，但不要立刻输出推荐。先调用 get_resume，针对仍不确定的信息用 2-4 个简短问题向我确认；待我回答后再调用 present_career_directions 给出推荐。",
  },

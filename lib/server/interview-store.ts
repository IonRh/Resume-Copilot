...
function ensureSchema(db: DatabaseSync) {
  ...
  `)
  for (const table of ["interview_sessions", "interview_agent_states", "interview_handoffs", "interview_reports"]) {
    try {
      db.exec(`ALTER TABLE ${table} ADD COLUMN owner TEXT NOT NULL DEFAULT 'admin'`)
    } catch {
      /* column already exists */
    }
  }
  db.exec(`
    CREATE INDEX IF NOT EXISTS idx_interview_sessions_owner_updated_at ON interview_sessions(owner, updated_at DESC);
    CREATE INDEX IF NOT EXISTS idx_interview_agent_states_owner_updated_at ON interview_agent_states(owner, updated_at DESC);
...
function ownedStorageKey(owner: string, key: string): string {
  return owner === "admin" ? key : `${owner}:${key}`
}
...
export function upsertInterviewSession(record: InterviewSessionRecord): Promise<InterviewSessionRecord> {
  return withLock(async () => {
    ...
    db.prepare(`
      INSERT INTO interview_sessions (id, owner, updated_at, record_json)
      VALUES (?, ?, ?, ?)
...
export function patchInterviewSession(
  ...
  return withLock(async () => {
    ...
    db.prepare(`
      ...
      WHERE id = ? AND owner = ?
    `).run(updatedAt, JSON.stringify(patched), id, await getCurrentUsername())
...
export function deleteInterviewSession(id: string): Promise<boolean> {
  return withLock(async () => {
    const db = await getDb()
    const owner = await getCurrentUsername()
    const result = db.prepare("DELETE FROM interview_sessions WHERE id = ? AND owner = ?").run(id, owner)
...
export function deleteInterviewAgentStates(keys: string[]): Promise<number> {
  return withLock(async () => {
    ...
    const db = await getDb()
    const owner = await getCurrentUsername()
    const dbKeys = keys.map((key) => ownedStorageKey(owner, key))
    const placeholders = dbKeys.map(() => "?").join(", ")
    const result = db.prepare(`DELETE FROM interview_agent_states WHERE owner = ? AND storage_key IN (${placeholders})`).run(owner, ...dbKeys)
...
export async function getInterviewAgentState(storageKey: string): Promise<PersistedAgentState | null> {
  const owner = await getCurrentUsername()
  const db = await getDb()
  const dbKey = ownedStorageKey(owner, storageKey)
  const row = db.prepare(`
    SELECT state_json AS value_json
    FROM interview_agent_states
    WHERE storage_key = ? AND owner = ?
  `).get(dbKey, owner) as JsonRow | undefined
...
export function saveInterviewAgentState(storageKey: string, state: PersistedAgentState): Promise<void> {
  return withLock(async () => {
    ...
    const dbKey = ownedStorageKey(owner, storageKey)
    db.prepare(`
      INSERT INTO interview_agent_states (storage_key, owner, updated_at, state_json)
      VALUES (?, ?, ?, ?)
      ON CONFLICT(storage_key) DO UPDATE SET
        updated_at = excluded.updated_at,
        state_json = excluded.state_json
        WHERE owner = excluded.owner
    `).run(dbKey, owner, new Date().toISOString(), JSON.stringify(state))
  })
}
...
export async function getInterviewHandoff(fromSessionId: string): Promise<InterviewRoundHandoff | null> {
  const owner = await getCurrentUsername()
  const db = await getDb()
  const dbKey = ownedStorageKey(owner, fromSessionId)
  const row = db.prepare(`
    SELECT value_json
    FROM interview_handoffs
    WHERE from_session_id = ? AND owner = ?
  `).get(dbKey, owner) as JsonRow | undefined
...
export function saveInterviewHandoff(handoff: InterviewRoundHandoff): Promise<InterviewRoundHandoff> {
  return withLock(async () => {
    const db = await getDb()
    const owner = await getCurrentUsername()
    const dbKey = ownedStorageKey(owner, handoff.fromSessionId)
    db.prepare(`
      INSERT INTO interview_handoffs (from_session_id, owner, updated_at, value_json)
      VALUES (?, ?, ?, ?)
      ON CONFLICT(from_session_id) DO UPDATE SET
        updated_at = excluded.updated_at,
        value_json = excluded.value_json
        WHERE owner = excluded.owner
    `).run(dbKey, owner, handoff.generatedAt || new Date().toISOString(), JSON.stringify(handoff))
...
export function deleteInterviewHandoff(fromSessionId: string): Promise<boolean> {
  return withLock(async () => {
    ...
    const owner = await getCurrentUsername()
    const dbKey = ownedStorageKey(owner, fromSessionId)
    const result = db.prepare("DELETE FROM interview_handoffs WHERE from_session_id = ? AND owner = ?").run(dbKey, owner)
    return Number(result.changes) > 0
  })
}
...
export async function getInterviewReport(campaignId: string): Promise<StoredCampaignReport | null> {
  const owner = await getCurrentUsername()
  const db = await getDb()
  const dbKey = ownedStorageKey(owner, campaignId)
  const row = db.prepare(`
    SELECT value_json
...
export function saveInterviewReport(report: StoredCampaignReport): Promise<StoredCampaignReport> {
  return withLock(async () => {
    const db = await getDb()
    const owner = await getCurrentUsername()
    const dbKey = ownedStorageKey(owner, report.campaignId)
    db.prepare(`
      INSERT INTO interview_reports (campaign_id, owner, updated_at, value_json)
      VALUES (?, ?, ?, ?)
      ON CONFLICT(campaign_id) DO UPDATE SET
        updated_at = excluded.updated_at,
        value_json = excluded.value_json
        WHERE owner = excluded.owner
    `).run(dbKey, owner, report.generatedAt || new Date().toISOString(), JSON.stringify(report))
    return clone(report)
  })
}
